...
4. **接口化DDL方法**：将DDL列变更方法接口化，提高代码一致性和可维护性
...
### 8.4 转换器优化
1. **更好地利用SQL Template**：IRToXXX转换器现在更好地利用了SQL Template中的方法，而不是直接拼接SQL字符串
...
6. **接口化转换器**：将源到IR和IR到目标的转换分别接口化，提高通用性和灵活性
7. **保留精度信息**：确保字段的精度(columnSize)和小数位数(ratio)信息在转换过程中不丢失
