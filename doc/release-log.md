...
- 重构失败重试：由队列改为直接通用，重试数据由目标改为源。
- 严重缺陷：修复 sql server CT 队列溢出丢数据风险。
- 优化：统一增量数据入队处理，提高可维护性。
- 缺陷：修复 sql server max 字段无法在 mysql 端建表问题。
- 优化：简化 sql server CT 快照保存方式。
- 功能调整：将原来的任务启动过后就不能编辑修改成运行中不能编辑
- 界面：
  - 将任务同步类型移动到添加任务界面
  - 将表同步进度列表放到概括标签页中
  - 列表数量逻辑重构，新增同步状态，速度，移除全量同步进度
  - 修复添加映射不会在映射列表刷新 bug
  - 调整卡片布局
...
- 优化 sql server ct 查询性能
- 界面优化：连接页面调整，任务卡片调整、任务编辑界面调整。
- 重构：快照保存机制。
