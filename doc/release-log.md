...
- 新功能：目标端 kafka 支持 DDL 写入
- 缺陷：阻止 kafka 建表行为
- 缺陷：修复 sql server 新建表直接同步自增字段报错问题。

## 2.14

- 优化：重构失败重做并增加全部重试功能。注意：不兼容此版本之前的错误数据。
- 缺陷：修复并发计数问题。
- 缺陷：修复目标字段叠加问题。
- 缺陷：修复 sql server BOOLEAN 类型对 BIT 的支持。

## 2.13.1

- 优化：解决计数过于频繁写库问题。
- 缺陷：解决不能立即停止 server ct 增量任务的问题。 
- 界面缺陷：解决任务按钮乱跳问题。
