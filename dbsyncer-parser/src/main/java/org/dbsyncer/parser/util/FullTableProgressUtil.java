...
public abstract class FullTableProgressUtil {
    ...
    /**
     * 移除历史任务级断点键（pageIndex / cursor / tableGroupIndex）。
     *
     * @param snapshot Meta.snapshot
     */
    public static void removeLegacyTaskBreakpointKeys(Map<String, String> snapshot) {
        if (snapshot == null) {
            return;
        }
        snapshot.remove(ParserEnum.PAGE_INDEX.getCode());
        snapshot.remove(ParserEnum.CURSOR.getCode());
        snapshot.remove(ParserEnum.TABLE_GROUP_INDEX.getCode());
    }
    ...
    /**
     * 本机派工中是否仍有未完成的全量分片（用于空闲节点拉起判定）。
     *
     * @param taskId 任务/Mapping ID
     * @param snapshot Meta.snapshot
     * @param locals 本机 Assignment
     * @return true 应拉起全量 Puller
     */
    public static boolean hasIncompleteLocalAssignment(String taskId, Map<String, String> snapshot,
                                                       List<WorkItemAssignment> locals) {
        if (CollectionUtils.isEmpty(locals) || StringUtil.isBlank(taskId)) {
            return false;
        }
        for (WorkItemAssignment assignment : locals) {
            if (assignment == null || !StringUtil.equals(taskId, assignment.getTaskId())) {
                continue;
            }
            String itemId = assignment.getItemId();
            if (StringUtil.isBlank(itemId) || WorkItemIds.isTaskLevelItem(taskId, itemId)) {
                continue;
            }
            if (!isDone(snapshot, itemId)) {
                return true;
