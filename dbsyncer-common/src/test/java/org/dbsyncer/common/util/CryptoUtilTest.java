...
public class CryptoUtilTest {
    ...
    public void testEncryptAndDecryptData() {
        ...
        // AES解密数据
        String decryptedData = CryptoUtil.decryptData(
                encryptedResult.get("encryptedData"),
                encryptedResult.get("iv"),
                aesKey
        );
    ...
    public void testBuildAndParseEncryptedRequest_PublicNetwork() {
        ...
        // 构建加密请求（公网场景需要私钥签名）
        String encryptedRequest = CryptoUtil.buildEncryptedRequest(
                testData, 
                rsaConfig.getPublicKey(), 
                rsaConfig.getPrivateKey(),
                null, // 公网场景不需要HMAC密钥
                isPublicNetwork
        );
        ...
        // 解析加密请求（公网场景使用公钥验证签名）
        Map<String, Object> decryptedData = CryptoUtil.parseEncryptedRequest(
                encryptedRequest, 
                rsaConfig.getPrivateKey(),
                rsaConfig.getPublicKey(),
                null, // 内网场景才需要HMAC密钥
