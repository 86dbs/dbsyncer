 ...
 */
package org.dbsyncer.connector.oceanbase.binlog;
...
import org.dbsyncer.connector.oceanbase.deserializer.DeleteDeserializer;
import org.dbsyncer.connector.oceanbase.deserializer.ExtDeleteDeserializer;
import org.dbsyncer.connector.oceanbase.deserializer.ExtUpdateDeserializer;
import org.dbsyncer.connector.oceanbase.deserializer.ExtWriteDeserializer;
import org.dbsyncer.connector.oceanbase.deserializer.UpdateDeserializer;
import org.dbsyncer.connector.oceanbase.deserializer.WriteDeserializer;
...
public class BinaryLogRemoteClient implements BinaryLogClient {
    ...
    private void openChannel() throws IOException {
        try {
            ...
            closeChannel(channel);
            throw new IOException("Failed to connect to OceanBase on " + hostname + ":" + port + ". Please make sure it's running.", e);
        }
    ...
    private boolean tryUpgradeToSSL(GreetingPacket greetingPacket) throws IOException {
        if (sslMode != SSLMode.DISABLED) {
            ...
            if (!serverSupportsSSL && (sslMode == SSLMode.REQUIRED || sslMode == SSLMode.VERIFY_CA || sslMode == SSLMode.VERIFY_IDENTITY)) {
                throw new IOException("OceanBase server does not support SSL");
            }
     ...
     * to "" (provided it's null) forcing OceanBase to send events starting from the oldest known binlog (keep in mind
    ...
    /**
     * @param gtidSetFallbackToPurged true if gtid_purged should be used as a fallback when gtidSet is set to "" and OceanBase server has purged
    ...
    /**
     * @param useBinlogFilenamePositionInGtidMode true if OceanBase server should start streaming events from a given {@link
