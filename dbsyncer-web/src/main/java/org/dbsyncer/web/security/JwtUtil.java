...
public class JwtUtil {
     ...
     * 
     * @param appId 业务系统标识
     * @param secret 密钥（建议使用系统配置的RSA私钥或公钥的hash值）
     * @return JWT Token
     */
     ...
     */
    public static TokenInfo verifyToken(String token, String secret) {
        try {
            ...
            tokenInfo.setAppId(payload.getString("appId"));
    ...
    public static String refreshToken(String token, String secret) {
        try {
            ...
            // 生成新token
            return generateToken(tokenInfo.getAppId(), secret);
        } catch (Exception e) {
